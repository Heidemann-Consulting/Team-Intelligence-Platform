...
// Updated: Modified handleSaveAskAIResponseAsNewDocument to check for existing document by name
// and update it if found, otherwise create a new document.
...
import { ContentItemDetail, ContentItemType, RunWorkflowResponse, ContentItemDuplicatePayload, AskAIRequestPayload, PaginatedResponse, ContentItemListed } from '@/types/api';
...
`;
  ...
  const handleAskAI = async () => {
    ...
    setShowAskAIResponseModal(true);

    try {
        const payload: AskAIRequestPayload = {
            current_document_content: contentForAI,
            user_query: askAiQuery,
            document_name: itemDetails?.name || newItemName || "Untitled Document"
        };

        const result = await contentService.askAI(payload);
        setAskAIResponse(result.ai_response || "AI returned no text content.");
  ...
  const handleSaveAskAIResponseAsNewDocument = async (documentName: string, contentToSave: string) => {
    ...
    try {
        ...
        // Check if a document with this name already exists for the current team
        ...
        if (existingDocs.items && existingDocs.items.length > 0) {
            // Ensure the found document is actually owned by the current team, and names match exactly.
            // Backend search with name_query might be a partial match, so frontend must verify exact name and ownership.
        ...
        if (existingTeamDoc) {
            // Document with this name exists for the team, save as new version
            ...
            const createPayload: ContentItemCreatePayload = {
                ...
                // template_id is not provided, backend handles this for non-template based creation
            };
            const createdDocMeta = await contentService.createItem(createPayload);

            // Save the AI response as the first version of this new document
            const versionPayload = { markdown_content: contentToSave };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-theme(space.24))]">
      ...
      { (itemDetails || (isCreatingNewItem && editorContent)) && (
        <div className="flex-shrink-0 my-4 p-4 bg-white rounded-lg shadow border border-ulacm-gray-100">
            <div className="flex items-center gap-3">
                ...
                >
                    {isAskingAI ? <LoadingSpinner size="sm" color="text-white" className="mr-2" /> : <Brain size={18} className="mr-1.5" />}
      ...
      {showAskAIResponseModal && (
        <AskAIResponseModal
            ...
            aiResponseContent={askAIResponse}
            error={askAIError}
