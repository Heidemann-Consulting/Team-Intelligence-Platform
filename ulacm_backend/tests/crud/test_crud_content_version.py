...
async def test_get_by_id(mock_db_session):
    ...
    mock_db_session.execute.return_value.scalar_one_or_none.return_value = expected_version

    # Call the function under test (which should now await the mock's result)
    found_version = await crud_content_version.get_by_id(db=mock_db_session, version_id=test_id)
    ...
    assert f"content_versions.version_id = '{test_id}'" in str(query.compile(compile_kwargs={"literal_binds": True}))

    # Test not found case
    mock_db_session.execute.reset_mock()
    mock_db_session.execute.return_value.scalar_one_or_none.reset_mock()
    mock_db_session.execute.return_value.scalar_one_or_none.return_value = None
    not_found_version = await crud_content_version.get_by_id(db=mock_db_session, version_id=PyUUID(uuid4().hex))
...
async def test_create_new_version_first(mock_db_session):
    ...
    compiled_update_str = str(update_stmt.compile(compile_kwargs={"literal_binds": True}))
    ...
    assert f"WHERE content_items.item_id = '{item_id_str_no_hyphen}'" in compiled_update_str
    ...
    assert created_version.version_id == new_version_id # Ensure ID was set by refresh mock
...
async def test_create_new_version_subsequent(mock_db_session):
    ...
    mock_db_session.execute = AsyncMock(side_effect=[mock_max_version_result, mock_update_result])
    ...
    max_version_query_str = str(max_version_query.compile(compile_kwargs={"literal_binds": True}))
    assert "max(content_versions.version_number)" in max_version_query_str.lower()
    # *** FIX: Check WHERE clause with hyphenless UUID ***
    assert f"WHERE content_versions.item_id = '{item_id_str_no_hyphen}'" in max_version_query_str
...
async def test_get_versions_for_item(mock_db_session):
    ...
    test_item_id_str_no_hyphen = str(test_item_id).replace('-', '') # For WHERE clause check
    version1 = create_test_content_version(version_number=1, version_id="cccccccc-1111-1111-1111-000000000001")
    version2 = create_test_content_version(version_number=2, version_id="cccccccc-1111-1111-1111-000000000002")
    ...
    count_query_str = str(mock_db_session.execute.await_args_list[0].args[0].compile(compile_kwargs={"literal_binds": True}))
    items_query_str = str(mock_db_session.execute.await_args_list[1].args[0].compile(compile_kwargs={"literal_binds": True}))

    # *** FIX: Check filters with hyphenless UUID ***
    assert f"WHERE content_versions.item_id = '{test_item_id_str_no_hyphen}'" in count_query_str
    assert f"WHERE content_versions.item_id = '{test_item_id_str_no_hyphen}'" in items_query_str
    # Check sorting
...
async def test_get_specific_version_by_number(mock_db_session):
    ...
    expected_version = create_test_content_version(item_id=str(test_item_id), version_number=version_number_to_find)
