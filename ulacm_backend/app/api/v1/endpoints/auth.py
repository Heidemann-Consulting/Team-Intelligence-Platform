...
from app.schemas.team import Team as TeamSchema
from app.schemas.msg import Msg # For the admin response
...
async def login_team_user(
    ...
    response.set_cookie(
        ...
        httponly=True,
        samesite="lax",
        # secure=True if settings.BACKEND_CORS_ORIGINS else False, # OLD LOGIC
        secure=False, # <-- CHANGE THIS TO False for local HTTP testing
...
async def logout_team_user(request: Request, response: Response):
    ...
    """
    remote_ip = request.client.host if request.client else "unknown"
    # Ideally, we'd know which team is logging out from the cookie/session before clearing it
    log.info(f"Team user logout request from IP {remote_ip}")
    response.delete_cookie(
        key="team_session_id",
        path="/",
        samesite="lax",
        # secure=True if settings.BACKEND_CORS_ORIGINS else False
...
@router.get(
    "/auth/me", # Note: Path relative to the router included in api.py
    response_model=TeamSchema,
    summary="Get Current Authenticated Team User",
    tags=["Authentication"] # Add to the Authentication group
)
async def read_current_team(
    ...
    """
    # The dependency already verifies the token and fetches the user.
    return current_team

@router.get(
    "/admin/auth/me", # Note: Path relative to the router included in api.py
    response_model=Msg,
    summary="Check Current Admin Session",
    tags=["Admin - Authentication"] # Separate tag for admin auth
)
async def read_current_admin(
    current_admin_payload: dict = Depends(get_current_admin_user) # Dependency verifies admin
):
    """
