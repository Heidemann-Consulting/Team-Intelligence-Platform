...
# Updated: Enhanced get_items_for_team_or_admin with name_glob_patterns filtering.
...
class CRUDContentItem(CRUDBase[ContentItem, ContentItemCreate, ContentItemUpdateMeta]):
    ...
    def _glob_to_sql_like(self, glob_pattern: str) -> str:
        ...
        # Escape LIKE's special characters '%' and '_' in the input glob pattern first
        sql_pattern = glob_pattern.replace('%', '\\%').replace('_', '\\_')
        # Convert glob wildcards '*' and '?' to SQL LIKE wildcards '%' and '_'
    ...
    async def get_items_for_team_or_admin(
        ...
        *,
        ...
        if name_query: # Simple substring search, ANDed with glob patterns if both present
        ...
        # Ensure 'rank' is only used if content_query is active and needs_version_join is true
