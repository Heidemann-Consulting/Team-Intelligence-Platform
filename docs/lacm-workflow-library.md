...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Primary Input: `LACM_ManualNewsInput_YYYY-MM-DD.md` (created from `LACM_ManualNewsInput`)
    * Context Documents (User selects all that apply):
        * `LACM_Strategy_*.md`
        * `LACM_Competitors_*.md`
        * `LACM_TargetMarkets_*.md`
...
inputDocumentSelectors: # User will select relevant documents based on these patterns
  - "LACM_ManualNewsInput_*.md" # Expecting one for the current day
  - "LACM_Strategy_*.md"
  - "LACM_Competitors_*.md"
  - "LACM_TargetMarkets_*.md"
inputDateSelector: null # User selects the relevant dated 'ManualNewsInput'
outputName: "LACM_DailyNewsAnalysis_{{InputFileName | replace: 'LACM_ManualNewsInput_', '' | replace: '.md', ''}}_{{Year}}-{{Month}}-{{Day}}.md" # Tries to use date from input filename
prompt: |
  ...
  **Context:**
  - Primary Input: News items are provided in a document matching the pattern "LACM_ManualNewsInput_*.md". This document contains manually gathered summaries or pasted text of news from the last 24-48 hours.
  - Assessment Criteria: Evaluate news relevance based on potential direct impact on objectives outlined in the "LACM_Strategy_*.md" document, activities mentioned in the "LACM_Competitors_*.md" document, and trends within the "LACM_TargetMarkets_*.md" document. All these context documents are part of the input.
  ...
  **Task:**
  ...
  2. From the "LACM_ManualNewsInput_*.md" document, select ONLY THE TOP 3-5 items with the HIGHEST potential relevance based on the Assessment Criteria found in the other provided context documents. Prioritize actionable information over general updates.
...
* **Description:** Processes team member inputs (from a TIP Document like `LACM_DailyLogInput_YYYY-MM-DD.md`) and contextual documents to create a structured daily log.
* **Ritual Relevance:** LACM Phase 1.2: Daily Review & Context Capture.
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Primary Input: `LACM_DailyLogInput_YYYY-MM-DD.md` (created from `LACM_DailyLogInput`)
    * Context Documents (User selects relevant ones):
        * `LACM_YYYY-MM-DD-1_DailyPlan.md` (Previous day's plan)
        * `LACM_DailyNewsAnalysis_YYYY-MM-DD.md` (Today's analysis, generated by Workflow 1)
* **TIP Process Workflow Definition:**
...
inputDocumentSelectors:
  - "LACM_DailyLogInput_*.md" # Primary input with team's raw notes
  - "LACM_*_DailyPlan.md" # Previous day's plan (user selects the correct one)
  - "LACM_DailyNewsAnalysis_*.md" # Today's news analysis (user selects the correct one)
inputDateSelector: null
outputName: "LACM_DailyLog_{{InputFileName | replace: 'LACM_DailyLogInput_', '' | replace: '.md', ''}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
  ...
  **Goal:** Consolidate and structure the essential events, outcomes, and insights from the team's input (found in "LACM_DailyLogInput_*.md") covering the last 24 hours, creating a clear record for knowledge preservation and planning. Use the previous day's plan ("LACM_*_DailyPlan.md") and today's news analysis ("LACM_DailyNewsAnalysis_*.md") for context.
  **Context:**
  - Primary Input Document (Team Notes): Content matching "LACM_DailyLogInput_*.md".
  - Supporting Context Documents:
    - Previous Day's Plan: Content matching "LACM_*_DailyPlan.md".
    - Today's News Analysis: Content matching "LACM_DailyNewsAnalysis_*.md".
  - Today's Date: {{CurrentDate_YYYY-MM-DD}}.

  **Task:**
  1. Carefully analyze all provided input documents.
  2. From the "LACM_DailyLogInput_*.md" document, extract and categorize information clearly under the predefined headings below. Focus on concrete facts and outcomes.
  3. Compare achievements against the "LACM_*_DailyPlan.md" (previous day's plan) if possible, and note alignment or deviations if explicitly mentioned in the input notes.
  4. Note any relevant links to the day's external context if mentioned in the input notes or found in "LACM_DailyNewsAnalysis_*.md".
  ...
  *(Summarize from "Open Points / Questions / Items for Next 24h Focus" in the input document)*
  ...
  - [Reference any item from the provided "LACM_DailyNewsAnalysis_*.md" or "Relevant External Context Items" in input that directly impacted the day's status or plans.]
  ```
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * `LACM_Strategy_*.md`
    * `LACM_Goals_Quarter_*.md` (current quarter)
    * `LACM_KW*_WeeklyPlanPreview_*.md` (current week)
    * `LACM_DailyLog_YYYY-MM-DD.md` (today's log, generated by Workflow 2)
    * `LACM_DailyNewsAnalysis_YYYY-MM-DD.md` (today's analysis, generated by Workflow 1)
    * `LACM_TaskList_*.md`
    * `LACM_AvailableTime_*.md` (today's or current week's)
* **TIP Process Workflow Definition:**
...
inputDocumentSelectors: # User selects all relevant documents
  - "LACM_Strategy_*.md"
  - "LACM_Goals_Quarter_*.md"
  - "LACM_KW*_WeeklyPlanPreview_*.md"
  - "LACM_DailyLog_*.md" # Today's log
  - "LACM_DailyNewsAnalysis_*.md" # Today's news analysis
  - "LACM_TaskList_*.md"
  - "LACM_AvailableTime_*.md"
inputDateSelector: null
outputName: "LACM_DailyPlan_{{InputFileName | regex_replace: '.*(LACM_DailyLog_|LACM_AvailableTime_)([^_]+_[^_]+_[^_]+).*', '$2'}}_{{Year}}-{{Month}}-{{Day}}.md" # Complex attempt to get date part
prompt: |
  ...
  **Goal:** Propose a prioritized, actionable plan for the next 24 hours that directly contributes to the current Weekly Objectives (from "LACM_KW*_WeeklyPlanPreview_*.md") and Quarterly Goals (from "LACM_Goals_Quarter_*.md"), considering today's context ("LACM_DailyLog_*.md", "LACM_DailyNewsAnalysis_*.md"), overall tasks ("LACM_TaskList_*.md"), and available capacity ("LACM_AvailableTime_*.md"). Also provide a brief forward look.
  **Context:**
  - Strategic Direction: Content from "LACM_Strategy_*.md", "LACM_Goals_Quarter_*.md".
  - Weekly Focus: Crucially, align with "LACM_KW*_WeeklyPlanPreview_*.md". Identify the current week number if possible from file names or content.
  - Today's Status: Content from "LACM_DailyLog_*.md" (Accomplishments, Blockers, Open Points).
  - External Factors: Content from "LACM_DailyNewsAnalysis_*.md".
  - Resources: Content from "LACM_TaskList_*.md" (Backlog), "LACM_AvailableTime_*.md" (Capacity for next 24h).
  - Today: {{CurrentDate_YYYY-MM-DD}}. Let's assume Current Week is {{CurrentWeekNumber}}.

  **Task:**
  1. **Analyze Inputs:** Synthesize information from all provided documents, paying special attention to the objectives in "LACM_KW*_WeeklyPlanPreview_*.md".
  2. **Identify Priorities:** Determine the most critical tasks based on:
      - Direct contribution to Weekly/Quarterly goals.
      - Urgency arising from "LACM_DailyLog_*.md" (e.g., resolving blockers).
      - Tasks from "LACM_TaskList_*.md" that align with weekly objectives.
  3. **Propose 24h Plan:** Draft a list of specific, achievable tasks for the next 24 hours.
      ...
      - **Crucially, compare proposed effort against "LACM_AvailableTime_*.md" for feasibility.**
  ...
  4. **Propose 7d Outlook:** Briefly list the 1-3 most important tasks/themes from the "LACM_KW*_WeeklyPlanPreview_*.md" that should be kept in focus over the next 7 days.
  5. **Propose 31d Outlook:** Briefly list 1-2 key strategic themes or milestones from "LACM_Goals_Quarter_*.md" or "LACM_Strategy_*.md" relevant in the next month.
  ...
  *Feasibility Check: Proposed effort roughly aligns with available time stated in "LACM_AvailableTime_*.md".*
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Multiple `LACM_DailyNewsAnalysis_YYYY-MM-DD.md` files from the past 7 days.
    * `LACM_Strategy_*.md`
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors:
  - "LACM_DailyNewsAnalysis_*.md" # User selects 5-7 relevant daily analyses for the week
  - "LACM_Strategy_*.md"
inputDateSelector: null # User manually selects the relevant week's documents
outputName: "LACM_KW{{CurrentWeekNumber}}_WeeklyAnalysis_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
  **Role:** You are an AI Analyst supporting the LACM process by synthesizing weekly external trends.
  **Goal:** Summarize the provided daily external analyses (from "LACM_DailyNewsAnalysis_*.md" documents), identify significant recurring themes or emerging trends/patterns, and assess their potential cumulative impact on our strategy (from "LACM_Strategy_*.md") to inform tactical planning for Week {{CurrentWeekNumber}}.
  **Context:**
  - Primary Input: Content from several "LACM_DailyNewsAnalysis_*.md" documents for the past week.
  - Strategic Lens: Assess findings against objectives and focus areas in the provided "LACM_Strategy_*.md" document.
  - Current Week: {{CurrentWeekNumber}}. Today's Date: {{CurrentDate_YYYY-MM-DD}}.

  **Task:**
  1. Analyze all provided "LACM_DailyNewsAnalysis_*.md" documents.
  ...
  4. **Synthesize Top 3-5 Insights:** Determine the most strategically significant themes or trends based on their potential impact (positive or negative) on the objectives outlined in "LACM_Strategy_*.md".
  ...
  *(Based on provided Daily News Analysis documents)*
  ...
  1.  **Theme/Trend:** [Concise description of the first major theme/trend]
      ...
      **Potential Strategic Impact (based on `LACM_Strategy_*.md`):** [Assess how this could affect goals in the provided strategy document]

  2.  **Theme/Trend:** [Concise description of the second major theme/trend]
      **Observations:** [...]
      **Potential Strategic Impact (based on `LACM_Strategy_*.md`):** [...]
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Multiple `LACM_DailyLog_YYYY-MM-DD.md` files from the past 7 days.
    * `LACM_KW*_WeeklyPlanPreview_*.md` (the plan for the week being reviewed).
    * `LACM_Processes_*.md` (optional, for context on documented processes).
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors:
  - "LACM_DailyLog_*.md" # User selects 5-7 daily logs for the week
  - "LACM_KW*_WeeklyPlanPreview_*.md" # User selects the plan for the week being reviewed
  - "LACM_Processes_*.md" # Optional, user selects if relevant
inputDateSelector: null
outputName: "LACM_KW{{CurrentWeekNumber}}_RetroPreparation_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
  **Role:** You are an AI Analyst supporting the LACM weekly retrospective by identifying patterns in operational data.
  **Goal:** Analyze the provided "LACM_DailyLog_*.md" documents for Week {{CurrentWeekNumber}}, compare against the objectives in "LACM_KW*_WeeklyPlanPreview_*.md", and identify recurring patterns (positive and negative) related to workflow, collaboration, and goal achievement. This will provide objective data points for the team retrospective. Reference "LACM_Processes_*.md" if provided and relevant.
  **Context:**
  - Primary Input: Content from "LACM_DailyLog_*.md" documents for the past 7 days.
  - Planned Work: Content from "LACM_KW*_WeeklyPlanPreview_*.md" (objectives set for this week).
  - Documented Processes (Optional): Content from "LACM_Processes_*.md".
  - Current Week: {{CurrentWeekNumber}}. Today's Date: {{CurrentDate_YYYY-MM-DD}}.

  **Task:**
  1. Analyze all provided "LACM_DailyLog_*.md" documents, focusing on sections: 'Accomplishments', 'Challenges & Blockers', 'Learnings & Insights'.
  ...
  4. **Correlate with Processes (If `LACM_Processes_*.md` is provided):** Where possible, tentatively link identified patterns (especially negative ones) to specific steps or areas in "LACM_Processes_*.md". Note if issues seem related to undocumented workflows.
  ...
  6. **Formulate Open Questions:** Based on the patterns, formulate 2-3 open-ended questions designed to spark discussion during the retrospective (e.g., "What contributed to the recurring success in X?", "What might be underlying the repeated challenges with Y?", "How could process Z (from `LACM_Processes_*.md` if provided) be adjusted based on observation W?").
  7. Generate the Markdown file.
  ...
  **Observed Challenges/Negative Patterns:**
  - [Recurring blocker/issue 1, e.g., "Delays in receiving input for task Z mentioned 3 times in logs"] (Note potential link to a process in "LACM_Processes_*.md" if provided and clear from logs)
  ...
  **Potential Discussion Questions:**
  ...
  3. [Optional: Open question about process (from `LACM_Processes_*.md` if provided) or collaboration based on data]
  ```
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * `LACM_KW*_WeeklyAnalysis_*.md` (current week's analysis)
    * `LACM_KW*_WeeklyRetroSummary_*.md` (current week's retro summary)
    * `LACM_Strategy_*.md`
    * `LACM_Goals_Quarter_*.md` (current quarter)
    * `LACM_TaskList_*.md`
    * `LACM_PrioritizedInnovationInitiatives_*.md` (optional)
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors:
  - "LACM_KW*_WeeklyAnalysis_*.md"
  - "LACM_KW*_WeeklyRetroSummary_*.md"
  - "LACM_Strategy_*.md"
  - "LACM_Goals_Quarter_*.md"
  - "LACM_TaskList_*.md"
  - "LACM_PrioritizedInnovationInitiatives_*.md" # Optional, user may not select if N/A
inputDateSelector: null
outputName: "LACM_KW{{CurrentWeekNumber + 1}}_WeeklyPlanPreview_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
  **Role:** You are an AI Planning Assistant supporting the LACM weekly planning cycle.
  **Goal:** Draft a focused and achievable preview of the main objectives and key tasks for the upcoming week (KW{{CurrentWeekNumber + 1}}), ensuring alignment with quarterly goals (from "LACM_Goals_Quarter_*.md"), strategic priorities (from "LACM_Strategy_*.md"), and incorporating insights from the previous week's analysis ("LACM_KW*_WeeklyAnalysis_*.md") and retrospective summary ("LACM_KW*_WeeklyRetroSummary_*.md"). Consider tasks from "LACM_TaskList_*.md" and initiatives from "LACM_PrioritizedInnovationInitiatives_*.md" if provided.
  **Context:**
  - Strategic Guidance: Content from "LACM_Strategy_*.md", "LACM_Goals_Quarter_*.md".
  - Key Inputs from Current Week (Week {{CurrentWeekNumber}}): "LACM_KW*_WeeklyAnalysis_*.md", "LACM_KW*_WeeklyRetroSummary_*.md".
  - Task & Initiative Context: "LACM_TaskList_*.md", "LACM_PrioritizedInnovationInitiatives_*.md" (if provided).
  ...
  **Task:**
  ...
  2. **Identify Strategic Themes:** Extract relevant objectives from "LACM_Goals_Quarter_*.md" and "LACM_Strategy_*.md" that require progress in KW{{CurrentWeekNumber + 1}}. Include prioritized innovation initiatives if provided and relevant.
  3. **Incorporate Recent Insights:** Explicitly consider the 'Discussion Points' from "LACM_KW*_WeeklyAnalysis_*.md" and any actionable improvement items from "LACM_KW*_WeeklyRetroSummary_*.md".
  ...
  5. **List Key Supporting Tasks/Projects:** Identify the most significant tasks or project milestones from "LACM_TaskList_*.md" or implied by the objectives that need to be tackled in KW{{CurrentWeekNumber + 1}}. Differentiate between ongoing work and new initiatives.
  6. **Generate the Markdown file.**
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Multiple `LACM_KW*_WeeklyAnalysis_*.md` files from the past 4-5 weeks.
    * `LACM_Strategy_*.md`
    * `LACM_Goals_Quarter_*.md` (current quarter)
    * `LACM_YYYY-MM-1_MonthlyAnalysisStrategyCheck.md` (previous month's, optional for trend comparison)
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors:
  - "LACM_KW*_WeeklyAnalysis_*.md" # User selects 4-5 weekly analyses for the month
  - "LACM_Strategy_*.md"
  - "LACM_Goals_Quarter_*.md" # Current quarter's goals
  - "LACM_*_MonthlyAnalysisStrategyCheck.md" # Optional: previous month's report
inputDateSelector: null
outputName: "LACM_{{Year}}-{{Month}}_MonthlyAnalysisStrategyCheck.md"
prompt: |
  **Role:** You are an AI Strategy Analyst supporting the LACM monthly review cycle.
  **Goal:** Synthesize the past month's weekly analyses (from "LACM_KW*_WeeklyAnalysis_*.md"), identify significant trends/deviations, and rigorously assess alignment with current strategy ("LACM_Strategy_*.md") and quarterly goals ("LACM_Goals_Quarter_*.md"). Highlight key points for strategic discussion. If the previous month's analysis ("LACM_*_MonthlyAnalysisStrategyCheck.md") is provided, use it for trend comparison.
  **Context:**
  - Primary Input: Content from "LACM_KW*_WeeklyAnalysis_*.md" documents for the specified month.
  - Strategic Framework: Content from "LACM_Strategy_*.md", "LACM_Goals_Quarter_*.md".
  - Historical Context (Optional): Content from a previous "LACM_*_MonthlyAnalysisStrategyCheck.md".
  - Current Month/Year: {{CurrentMonthName}} {{CurrentYear}}. Assume the Quarter from the "LACM_Goals_Quarter_*.md" filename (e.g., Q{{CurrentQuarterNumber}}).

  **Task:**
  1. **Analyze Weekly Inputs:** Review all provided "LACM_KW*_WeeklyAnalysis_*.md" documents.
  ...
  3. **Assess Goal Progress:** Evaluate progress towards the objectives listed in "LACM_G_Quarter_*.md" based on the cumulative information from the weekly analyses. Note areas clearly on track, lagging, or facing significant challenges.
  4. **Perform Strategy Alignment Check:** Explicitly compare the identified monthly trends and goal progress against the key pillars/objectives defined in "LACM_Strategy_*.md". Are we still aligned? Are strategic assumptions holding?
  ...
  *(Max 3-5 key trends)*
  ...
  ## 2. Progress Towards Quarterly Goals (Q{{CurrentQuarterNumber}} {{CurrentYear}} from "LACM_Goals_Quarter_*.md")
  - **Objective:** [Objective from "LACM_Goals_Quarter_*.md"]
  ...
  - **Objective:** [...]
    *Status:* [...]
    *Observations:* [...]
  *(Cover all current Quarter Goals based on provided "LACM_Goals_Quarter_*.md")*

  ## 3. Strategic Alignment Assessment
  *(Based on comparison with provided "LACM_Strategy_*.md")*
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * User selects multiple older, detailed TIP documents (e.g., `LACM_DailyLog_YYYY-MM-DD.md`, `LACM_KWXX_WeeklyAnalysis.md` from a specific past month).
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors: # User selects a batch of older documents to condense
  - "LACM_DailyLog_*.md"
  - "LACM_KW*_WeeklyAnalysis_*.md"
  - "LACM_DailyNewsAnalysis_*.md"
inputDateSelector: null # User should select documents from a specific past period.
outputName: "LACM_CondensedContext_{{InputFileName | replace: 'LACM_', '' | truncate: 30, ''}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Multiple `LACM_KW*_WeeklyRetroSummary_*.md` files from the past 4-5 weeks.
    * `LACM_Processes_*.md` (current documented processes).
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors:
  - "LACM_KW*_WeeklyRetroSummary_*.md" # User selects 4-5 weekly retro summaries for the month
  - "LACM_Processes_*.md" # The current documented processes
inputDateSelector: null
outputName: "LACM_{{Year}}-{{Month}}_ProcessRetroPreparation.md"
prompt: |
  **Role:** You are an AI Process Analyst supporting the LACM monthly process review.
  **Goal:** Analyze the provided "LACM_KW*_WeeklyRetroSummary_*.md" documents for the month of {{CurrentMonthName}} {{CurrentYear}}. Identify recurring feedback, suggestions, or issues specifically related to the documented workflows in the provided "LACM_Processes_*.md" document. This will provide structured input for the team's process improvement discussion.
  **Context:**
  - Primary Input: Content from "LACM_KW*_WeeklyRetroSummary_*.md" documents. Focus on sections about challenges and improvement ideas.
  - Process Documentation: Content from "LACM_Processes_*.md".
  - Current Month/Year: {{CurrentMonthName}}, {{CurrentYear}}.

  **Task:**
  1. Analyze the relevant sections (challenges, improvement ideas, action items) of all provided "LACM_KW*_WeeklyRetroSummary_*.md" documents.
  2. **Filter for Process Relevance:** Extract only those points that directly mention, criticize, or suggest changes to a specific process or step documented in "LACM_Processes_*.md", or highlight a clear gap where a documented process might be needed.
  3. **Group by Process Area:** Group the extracted points according to the relevant process name or area from "LACM_Processes_*.md".
  ...
  *Summary of process-related feedback from weekly retrospectives, cross-referenced with "LACM_Processes_*.md".*

  ## Process Area: [Name of Process 1 from "LACM_Processes_*.md"]
  ...
  ## Process Area: [Name of Process 2 from "LACM_Processes_*.md"]
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * Multiple `LACM_YYYY-MM_MonthlyAnalysisStrategyCheck.md` files for the quarter.
    * `LACM_Strategy_*.md`
    * `LACM_SalesHandbook_*.md` (optional)
    * The expiring `LACM_Goals_Quarter_Q[CurrentQuarter]_[Year].md`
    * `LACM_PrioritizedInnovationInitiatives_*.md` (optional)
...
inputDocumentSelectors: # User selects all relevant documents for the quarter
  - "LACM_*_MonthlyAnalysisStrategyCheck.md"
  - "LACM_Strategy_*.md"
  - "LACM_SalesHandbook_*.md" # Optional
  - "LACM_Goals_Quarter_*.md" # Expiring and potentially next quarter's draft if iterative
  - "LACM_PrioritizedInnovationInitiatives_*.md" # Optional
  - "LACM_QuarterlyPerformanceData_*.md" # User-created doc with metrics
inputDateSelector: null
outputName: "LACM_Q{{CurrentQuarterNumber}}_{{Year}}_ReviewPreparation_DraftOKRs_Q{{CurrentQuarterNumber + 1}}.md"
prompt: |
  **Role:** You are an AI Strategy Analyst supporting the LACM quarterly review and planning cycle.
  **Goal:** Synthesize the concluding quarter's (Q{{CurrentQuarterNumber}} {{CurrentYear}}) performance and strategic context based on provided monthly reports and performance data. Evaluate progress against Q{{CurrentQuarterNumber}} goals (OKRs from "LACM_Goals_Quarter_Q{{CurrentQuarterNumber}}_{{Year}}.md"). Assess strategic alignment based on "LACM_Strategy_*.md" and "LACM_SalesHandbook_*.md" (if provided). Propose data-informed, draft Objectives and Key Results (OKRs) for the upcoming quarter (Q{{CurrentQuarterNumber + 1}}) considering "LACM_PrioritizedInnovationInitiatives_*.md" (if provided).
  **Context:**
  - Monthly Summaries: Content from "LACM_*_MonthlyAnalysisStrategyCheck.md" for Q{{CurrentQuarterNumber}}.
  - Strategic Framework: Content from "LACM_Strategy_*.md", "LACM_SalesHandbook_*.md".
  - Concluding Goals: Content from "LACM_Goals_Quarter_Q{{CurrentQuarterNumber}}_{{Year}}.md".
  - Future Focus (Optional): Content from "LACM_PrioritizedInnovationInitiatives_*.md".
  ...
  **Task:**
  ...
  2. **Evaluate Q{{CurrentQuarterNumber}} OKR Achievement:** For each Objective and Key Result in "LACM_Goals_Quarter_Q{{CurrentQuarterNumber}}_{{Year}}.md", assess the final status (e.g., Achieved, Partially Achieved, Not Achieved) and briefly note key contributing factors or reasons based on the monthly analyses.
  3. **Assess Strategic Alignment & Validity:** Review the findings from Task 1 & 2 against "LACM_Strategy_*.md". Highlight areas where strategic assumptions were validated or challenged. Suggest specific sections of "LACM_Strategy_*.md" or "LACM_SalesHandbook_*.md" that may warrant discussion or updates.
  4. **Incorporate Innovation Focus:** Consider "LACM_PrioritizedInnovationInitiatives_*.md" if provided; identify which initiatives are ready for potential inclusion in the next quarter's goals.
  5. **Draft Q{{CurrentQuarterNumber + 1}} OKRs:** Based on the analysis (especially strategic gaps/opportunities and unfinished business from Q{{CurrentQuarterNumber}}), propose 3-5 ambitious but achievable Objectives for Q{{CurrentQuarterNumber + 1}}. For each Objective, propose 2-4 specific, measurable, achievable, relevant, time-bound (SMART) Key Results. Ensure these draft OKRs clearly contribute to the overarching "LACM_Strategy_*.md".
  ...
  *(Based on provided "LACM_Goals_Quarter_Q{{CurrentQuarterNumber}}_{{Year}}.md" and performance data)*
  ...
  *(Based on comparison with "LACM_Strategy_*.md" and "LACM_SalesHandbook_*.md")*
  - [Assessment of how Q{{CurrentQuarterNumber}} outcomes impact a strategic assumption from "LACM_Strategy_*.md"]
  - [Suggestion: Review section X of "LACM_Strategy_*.md" based on finding Y]
  ...
  *(Considering strategic alignment, Q{{CurrentQuarterNumber}} outcomes, and Innovation Initiatives if provided)*
  - **Objective 1:** [Proposed Objective - Clear, ambitious, qualitative]
    *Alignment:* [Links to Strategy Goal X from "LACM_Strategy_*.md"]
    *Key Results:*
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * `LACM_QuarterlyReviewPrep_*.md` (or monthly/weekly analyses)
    * `LACM_Competitors_*.md`
    * `LACM_CustomerFeedback_*.md`
    * `LACM_IdeaBacklog_*.md` (to avoid duplicates)
    * `LACM_Strategy_*.md`
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors: # User selects relevant context documents
  - "LACM_*Analysis*.md" # Various analysis reports
  - "LACM_Competitors_*.md"
  - "LACM_CustomerFeedback_*.md"
  - "LACM_IdeaBacklog_*.md"
  - "LACM_Strategy_*.md"
inputDateSelector: null
outputName: "LACM_NewIdeas_Q{{CurrentQuarterNumber}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
  **Role:** You are an AI Ideation Catalyst supporting the LACM innovation process.
  **Goal:** Generate 5-10 concrete, relevant, and potentially innovative ideas for new products, services, process improvements, or strategic approaches. Base these ideas ONLY on the analysis of the provided input documents: recent performance/trend reports (various *Analysis*.md files), "LACM_Competitors_*.md", "LACM_CustomerFeedback_*.md", "LACM_Strategy_*.md". Check "LACM_IdeaBacklog_*.md" to avoid duplicating existing ideas.
  **Context:**
  - Performance & Trends: Content from various *Analysis*.md documents provided.
  - Market & Customer: Content from "LACM_Competitors_*.md", "LACM_CustomerFeedback_*.md".
  - Internal State: Content from "LACM_IdeaBacklog_*.md" (for checking existing ideas), "LACM_Strategy_*.md" (for identifying gaps or areas needing innovation).

  **Task:**
  1. **Analyze Context Deeply:** Review all provided input documents to identify:
      - Unmet customer needs or recurring complaints (from "LACM_CustomerFeedback_*.md").
      - Competitor weaknesses or market gaps (from "LACM_Competitors_*.md", analysis documents).
      ...
      - Areas where "LACM_Strategy_*.md" indicates a need for new approaches or has gaps.
  2. **Generate Diverse Ideas:** Based on this analysis, generate 5-10 distinct ideas. Aim for a mix covering: New Product/Service concepts, Enhancements, Process Improvements, Novel Strategic Approaches. Do not suggest ideas already listed in the "LACM_IdeaBacklog_*.md" document.
  3. **Formulate Ideas Clearly:** For each idea, provide:
      ...
      - The primary source of inspiration from the input documents (e.g., "Inspired by customer feedback on X in `LACM_CustomerFeedback.md`", "Addresses competitor Y's weakness noted in `LACM_Competitors.md`").
  ...
  *(Repeat for 5-10 ideas)*
  ```
  **Constraints:** Ideas should be concrete. Ensure ideas are relevant to the business context defined in "LACM_Strategy_*.md". Aim for novelty where possible. Clearly state the inspiration/rationale based ONLY on the provided documents.
  {{DocumentContext}}
```
```
...
* **Expected Input TIP Document Name(s) / Template Origin:**
    * All `LACM_Q*_ReviewPreparation_*.md` files for the year.
    * `LACM_YYYY_AnnualFinancialReport.md` (if created as a TIP doc by user)
    * User-created TIP documents summarizing market research.
    * The expiring `LACM_Strategy_*.md`.
* **TIP Process Workflow Definition:**
```yaml
...
inputDocumentSelectors: # User selects all relevant documents for the year
  - "LACM_Q*_ReviewPreparation_*.md" # All quarterly review preps
  - "LACM_*_AnnualFinancialReport.md" # Optional, if user created this
  - "LACM_MarketResearchSummary_*.md" # Optional, user-created
  - "LACM_Strategy_*.md" # The strategy document for the year being reviewed
inputDateSelector: null
outputName: "LACM_{{Year}}_AnnualStrategyAnalysisInput.md"
prompt: |
  **Role:** You are an AI Senior Strategy Analyst supporting the LACM annual strategic review for the year {{CurrentYear}}.
  **Goal:** Synthesize the entire year's performance, learnings, and market context based on the provided quarterly review preparation documents, financial reports (if any), market research summaries (if any), and the expiring strategy document ("LACM_Strategy_*.md" for {{CurrentYear}}). Critically evaluate the effectiveness and ongoing validity of the current strategy. Identify major strategic challenges and opportunities to support leadership's decision-making for the upcoming year.
  **Context:**
  - Quarterly Performance: Content from "LACM_Q*_ReviewPreparation_*.md" documents for {{CurrentYear}}.
  - Financials (If provided): Content from "LACM_*_AnnualFinancialReport.md".
  - Market (If provided): Content from "LACM_MarketResearchSummary_*.md".
  - Current Strategy: The expiring "LACM_Strategy_*.md" for {{CurrentYear}}.
  - Completed Year: {{CurrentYear}}.

  **Task:**
  1. **Synthesize Annual Performance:** Analyze all inputs to identify the most significant strategic outcomes, trends, successes, and failures over the entire year. Compare overall performance against the goals stated in the expiring "LACM_Strategy_*.md".
  2. **Critically Evaluate Current Strategy:** Assess each major section/pillar of the expiring "LACM_Strategy_*.md". Based on the year's evidence from the inputs, which assumptions held true? Which were challenged? Which strategic initiatives were most/least effective? Is the overall strategic logic still sound given the current market context (as described in inputs)?
  ...
  *Based on analysis of provided quarterly reviews, financial data (if any), market research (if any), and the {{CurrentYear}} strategy document.*
  ...
  ## 2. Critical Evaluation of {{CurrentYear}} Strategy (from "LACM_Strategy_*.md")
...
* **Expected Input TIP Document Name(s) / Template Origin:** User creates a TIP Document (e.g., `LACM_PairInput_DraftProductDescription_YYYY-MM-DD.md`) containing their specific request (topic, keywords, outline).
...
inputDocumentSelectors:
  - "LACM_PairInput_*.md"
inputDateSelector: null
outputName: "LACM_DraftOutput_from_{{InputFileName | replace: 'LACM_PairInput_', '' | truncate: 30, ''}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
...
* **Expected Input TIP Document Name(s) / Template Origin:** User creates a TIP Document (e.g., `LACM_PairInput_BrainstormSolutionsForX_YYYY-MM-DD.md`) detailing the brainstorming subject.
...
inputDocumentSelectors:
  - "LACM_PairInput_*.md"
inputDateSelector: null
outputName: "LACM_BrainstormOutput_from_{{InputFileName | replace: 'LACM_PairInput_', '' | truncate: 30, ''}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
...
* **Expected Input TIP Document Name(s) / Template Origin:** User creates a TIP Document (e.g., `LACM_PairInput_AnalyzeFeedback_YYYY-MM-DD.md`) with the text and the analysis request.
...
inputDocumentSelectors:
  - "LACM_PairInput_*.md"
inputDateSelector: null
outputName: "LACM_AnalysisOutput_from_{{InputFileName | replace: 'LACM_PairInput_', '' | truncate: 30, ''}}_{{Year}}-{{Month}}-{{Day}}.md"
prompt: |
...
* TIP will then prompt you to select the input TIP Document(s) based on the `inputDocumentSelectors` defined in the workflow.
    * Ensure your input TIP Documents are named according to conventions that match the selectors (e.g., a document named `LACM_ManualNewsInput_2025-05-11.md` for the `LACM_Workflow_DailyNewsAnalysis`).
    * For "Pair Working" workflows (like `LACM_Workflow_DraftSection`), you will typically create a new TIP Document (e.g., `LACM_PairInput_MyTopic_YYYY-MM-DD.md`) containing the specific text or instructions for the LLM, then select this document as input.
